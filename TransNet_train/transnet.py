...
class TransNetV2(tf.keras.Model):
    ...
    def __init__(self, F=16, L=3, S=2, D=256,
        ...
        # Add New Similarity Layer
        self.entropy_sim_layer = EntropySimilarity() if use_entropy_similarity else None
...
@gin.configurable(denylist=["name"])
class TransNetV2_modify(tf.keras.Model):
    def __init__(self, F=16, L=3, S=2, D=256,
                 use_resnet_features=False,
                 use_many_hot_targets=False,
                 use_frame_similarity=False,
                 ...
                 use_color_histograms=False,
                 use_entropy_similarity=False,
                 name="TransNetV2_modify"):
        super(TransNetV2_modify, self).__init__(name=name)

        self.resnet_layers = ResNetFeatures() if use_resnet_features else (lambda x, training=False: x / 255.)
        self.blocks = [StackedDDCNNV2_v1(n_blocks=S, filters=F, stochastic_depth_drop_prob=0., name="SDDCNN_1")]
        self.blocks += [StackedDDCNNV2_v2(n_blocks=S, filters=F * 2**i, name="SDDCNN_{:d}".format(i + 1)) for i in range(1, L)]
        self.fc1 = tf.keras.layers.Dense(D, activation=tf.nn.relu)
        self.cls_layer1 = tf.keras.layers.Dense(1, activation=None)
        self.cls_layer2 = tf.keras.layers.Dense(1, activation=None) if use_many_hot_targets else None
        self.frame_sim_layer = FrameSimilarity() if use_frame_similarity else None
    ...
    def call(self, inputs, training=False):
        ...
        if self.entropy_sim_layer is not None:
            x = tf.concat([self.entropy_sim_layer(inputs), x], 2)
...
@gin.configurable(allowlist=["shortcut", "use_octave_conv", "pool_type", "stochastic_depth_drop_prob"])
class StackedDDCNNV2_v1(tf.keras.layers.Layer):

    def __init__(self, n_blocks, filters, shortcut=False, use_octave_conv=False, pool_type="max",
                 stochastic_depth_drop_prob=0., name="StackedDDCNN"):
        super(StackedDDCNNV2_v1, self).__init__(name=name)
        assert pool_type == "max" or pool_type == "avg"
        if use_octave_conv and pool_type == "max":
            print("WARN: Octave convolution was designed with average pooling, not max pooling.")
        ...
        self.blocks = [DilatedDCNNV2_v1(filters, octave_conv=use_octave_conv,
                                     activation=tf.nn.relu if i != n_blocks else None,
                                     name="DDCNN_{:d}".format(i)) for i in range(1, n_blocks + 1)]
        self.pool = tf.keras.layers.MaxPool3D(pool_size=(1, 2, 2)) if pool_type == "max" else \
            tf.keras.layers.AveragePooling3D(pool_size=(1, 2, 2))
...
@gin.configurable(allowlist=["shortcut", "use_octave_conv", "pool_type", "stochastic_depth_drop_prob"])
class StackedDDCNNV2_v2(tf.keras.layers.Layer):

    def __init__(self, n_blocks, filters, shortcut=False, use_octave_conv=False, pool_type="max",
                 stochastic_depth_drop_prob=0., name="StackedDDCNN"):
        super(StackedDDCNNV2_v2, self).__init__(name=name)
        assert pool_type == "max" or pool_type == "avg"
        if use_octave_conv and pool_type == "max":
            print("WARN: Octave convolution was designed with average pooling, not max pooling.")
        ...
        self.blocks = [DilatedDCNNV2_v2(filters, octave_conv=use_octave_conv,
                                     activation=tf.nn.relu if i != n_blocks else None,
                                     name="DDCNN_{:d}".format(i)) for i in range(1, n_blocks + 1)]
        self.pool = tf.keras.layers.MaxPool3D(pool_size=(1, 2, 2)) if pool_type == "max" else \
            tf.keras.layers.AveragePooling3D(pool_size=(1, 2, 2))
...
@gin.configurable(allowlist=["batch_norm"])
class DilatedDCNNV2_v1(tf.keras.layers.Layer):

    def __init__(self, filters, batch_norm=False, activation=None, octave_conv=False, name="DilatedDCNN"):
        super(DilatedDCNNV2_v1, self).__init__(name=name)
        assert not (octave_conv and batch_norm)

        self.conv1 = Conv3DConfigurable_v1(filters, 1, use_bias=not batch_norm, octave=octave_conv, name="Conv3D_1")
        self.conv2 = Conv3DConfigurable_v1(filters, 2, use_bias=not batch_norm, octave=octave_conv, name="Conv3D_2")
        self.conv3 = Conv3DConfigurable_v1(filters, 4, use_bias=not batch_norm, octave=octave_conv, name="Conv3D_4")
        self.conv4 = Conv3DConfigurable_v1(filters, 6, use_bias=not batch_norm, octave=octave_conv, name="Conv3D_6")
        self.octave = octave_conv
...
@gin.configurable(allowlist=["batch_norm"])
class DilatedDCNNV2_v2(tf.keras.layers.Layer):

    def __init__(self, filters, batch_norm=False, activation=None, octave_conv=False, name="DilatedDCNN"):
        super(DilatedDCNNV2_v2, self).__init__(name=name)
        assert not (octave_conv and batch_norm)

        self.conv1 = Conv3DConfigurable(filters, 1, use_bias=not batch_norm, octave=octave_conv, name="Conv3D_1")
        self.conv2 = Conv3DConfigurable(filters, 2, use_bias=not batch_norm, octave=octave_conv, name="Conv3D_2")
        self.conv3 = Conv3DConfigurable(filters, 4, use_bias=not batch_norm, octave=octave_conv, name="Conv3D_4")
        self.conv4 = Conv3DConfigurable(filters, 6, use_bias=not batch_norm, octave=octave_conv, name="Conv3D_6")
        self.octave = octave_conv
...
@gin.configurable(allowlist=["separable", "kernel_initializer"])
class Conv3DConfigurable_v1(tf.keras.layers.Layer):
    ...
    def __init__(self,
                 ...
                 kernel_initializer="glorot_uniform",
                 name="Conv3D"):
        super(Conv3DConfigurable_v1, self).__init__(name=name)
        assert not (separable and octave)

        if separable:
            # (2+1)D convolution https://arxiv.org/pdf/1711.11248.pdf
            conv1 = tf.keras.layers.Conv3D(2 * filters, kernel_size=(1, 5, 5), dilation_rate=(1, 1, 1),
                                           padding="SAME", activation=None, use_bias=False,
                                           name="conv_spatial", kernel_initializer=kernel_initializer)
            conv2 = tf.keras.layers.Conv3D(filters, kernel_size=(5, 1, 1), dilation_rate=(dilation_rate, 1, 1),
                                           padding="SAME", activation=None, use_bias=use_bias, name="conv_temporal",
                                           kernel_initializer=kernel_initializer)
            self.layers = [conv1, conv2]
        elif octave:
...
@gin.configurable(allowlist=["lookup_window", "output_dim"])
class EntropySimilarity(tf.keras.layers.Layer):
    # 각 프레임마다 R, G, B 채널을 분리 -> 각각 히스토그램을 생성
    # 각 히스토그램에 대해 엔트로피 값을 계산
    # 하나로 연결
    # => 프레임의 유사도를 비교

    def __init__(self, lookup_window=101, output_dim=None, name="EntropySimilarity"):
        super(EntropySimilarity, self).__init__(name=name)
