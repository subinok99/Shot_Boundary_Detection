...
class TransNetV2:
    def __init__(self, model_dir=None):
        if model_dir is None:
            model_dir = os.path.join(os.path.dirname(__file__), "weight/")
    ...
    def predict_frames(self, frames: np.ndarray):
        ...
        #data = open('waveform/summe_22' + ".txt", 'a')
        #for idx in range(len(single_frame_pred)):
        #    data.write(str(single_frame_pred[idx]) + ",")
        #data.close()
